...
type FilterPanelProps = Omit<GeneratorTabProps, "generatedSets" | "justSavedIndex" | "onCopy" | "onOpenReport" | "onSaveNumber">;

function FilterPanel({
  user, gridModes, fixedNumbers, excludedNumbers,
  oddEvenRatio, sumPreset, customMinSum, customMaxSum,
  generateCount, showAdvancedFilters, consecutiveLimit,
  endingSumPreset, customMinEndingSum, customMaxEndingSum,
  ...
  onGridClick, onResetGrid, onGenerate, onLoginGate,
}: FilterPanelProps) {
  return (
    <div className="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm space-y-5 relative">
      ...
      <div>
        ...
        <div className="grid grid-cols-4 gap-1.5">
          {[
            { label: "전체", value: "all" }, { label: "홀3:짝3", value: "3:3" },
            { label: "홀2:짝4", value: "2:4" }, { label: "홀4:짝2", value: "4:2" },
            { label: "홀1:짝5", value: "1:5" }, { label: "홀5:짝1", value: "5:1" },
            { label: "홀0:짝6", value: "0:6" }, { label: "홀6:짝0", value: "6:0" },
          ].map((item) => (
      ...
      {/* 데스크톱: 필터 패널 내부 하단 생성 버튼 */}
      <button onClick={onGenerate}
        className="hidden md:flex w-full py-3.5 bg-indigo-600 hover:bg-indigo-700 active:scale-[0.98] text-white rounded-xl text-sm font-bold items-center justify-center gap-2 transition-all shadow-sm">
...
function ResultPanel({
  generatedSets, justSavedIndex, user, onCopy, onOpenReport, onSaveNumber, onLoginGate,
}: Pick<GeneratorTabProps, "generatedSets" | "justSavedIndex" | "user" | "onCopy" | "onOpenReport" | "onSaveNumber" | "onLoginGate">) {
  if (generatedSets.length === 0) return null;
  return (
...
export default function GeneratorTab(props: GeneratorTabProps) {
  const { generatedSets, justSavedIndex, onCopy, onOpenReport, onSaveNumber, onGenerate, ...rest } = props;

  const resultProps = { generatedSets, justSavedIndex, user: props.user, onCopy, onOpenReport, onSaveNumber, onLoginGate: props.onLoginGate };
  const filterProps: FilterPanelProps = { ...rest, onGenerate };

  return (
    <>
      {/* ── 모바일: 단일 컬럼 (생성 버튼 → 결과 → 필터) ── */}
      ...
      {/* ── 데스크톱: 2컬럼 (필터 좌 sticky, 결과 우) ── */}
      <div className="hidden md:grid md:grid-cols-[420px_1fr] lg:grid-cols-[460px_1fr] gap-6 items-start">
        {/* 좌: 필터 패널 (sticky) */}
        ...
        <div className="space-y-4">
          <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm">
            <div className="flex items-center gap-3 mb-5">
              ...
              <div>
                <h2 className="font-bold text-base text-slate-900">번호 추천 생성</h2>
                <p className="text-xs text-slate-400">좌측 필터 조건을 적용해 6/45 번호를 추출합니다</p>
              </div>
            </div>
            <button onClick={onGenerate}
              className="w-full py-4 bg-indigo-600 hover:bg-indigo-700 active:scale-[0.98] text-white rounded-xl text-base font-bold flex items-center justify-center gap-2 transition-all shadow-sm">
          ...
          {generatedSets.length === 0 && (
            <div className="bg-white border border-dashed border-slate-200 rounded-2xl p-12 text-center">
              <Dices className="w-10 h-10 text-slate-200 mx-auto mb-3" />
              <p className="text-sm font-semibold text-slate-400">아직 생성된 번호가 없습니다</p>
              <p className="text-xs text-slate-400 mt-1">좌측에서 필터를 설정하고 버튼을 눌러보세요.</p>
            </div>
          )}
        </div>
      </div>
