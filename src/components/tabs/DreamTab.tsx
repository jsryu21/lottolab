...
export default function DreamTab({
  ...
  const ResultCard = dreamLoading ? (
    <div className="bg-white border border-slate-200 rounded-2xl p-10 shadow-sm text-center">
      ...
      <p className="text-[11px] text-slate-400 max-w-xs mx-auto">Gemini AI가 꿈의 상징을 분석하고 있습니다.</p>
    </div>
  ) : dreamResult ? (
    ...
    <div className="bg-white border border-slate-200 rounded-2xl p-10 shadow-sm text-center">
      ...
      <p className="text-[11px] text-slate-400 mt-1">꿈 내용을 입력하고 분석 버튼을 눌러보세요.</p>
    </div>
  );

  const HistoryCard = isSupabaseConfigured ? (
    <div className="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm">
  ...
  return (
    <>
      ...
      {/* ── 데스크톱: 2컬럼 (입력 좌, 결과+히스토리 우) ── */}
      <div className="hidden md:grid md:grid-cols-2 gap-6 items-start">
        <div className="sticky top-[121px] space-y-4">
          {InputCard}
        </div>
